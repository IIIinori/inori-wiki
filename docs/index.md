---
layout: home

hero:
  name: Inori Wiki
  text: Inori 系列插件文档入口
  tagline: 在这里选择你需要查看的 Inori 系列插件文档。
features:
  - title: InoriLoot
    details: 战利品、钥匙、掉落池、仓库、保险箱、Raid、常驻地图、队伍与撤离结算。
    link: https://iiiinori.github.io/inori-loot-wiki/start/quickstart
    linkText: 进入 InoriLoot 文档
  - title: InoriLootShop
    details: InoriLoot 的商店与交易行附属插件，提供 GUI 商店、玩家交易行、动态价值、自定义货币。
    link: https://iiiinori.github.io/inori-lootshop-wiki/start/quickstart
    linkText: 进入 InoriLootShop 文档
---
